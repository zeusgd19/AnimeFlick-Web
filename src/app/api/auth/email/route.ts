...
export async function PUT(req: NextRequest) {
    ...
    if (!body || !body.email) return NextResponse.json({ message: "Invalid body, expected { email: string }" }, { status: 400 });
    ...
    const res = await fetch(`${USER_API}/auth/email`, {
        method: "PUT",
        headers: {
    ...
    if (res.ok) {
        // Update the af_user cookie with new email
